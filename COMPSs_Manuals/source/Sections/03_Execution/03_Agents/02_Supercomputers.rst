...
Similar to Section :ref:`Sections/03_Execution/01_Local:Executing COMPSs applications` for Master-Worker deployments, this section is intended to walk you through the COMPSs usage with agents in Supercomputers. All the configuration and commands to install COMPSs on the Supercomputer, load the environment and submitting a job remain exactly the same as described in Sections :ref:`Sections/03_Execution/02_Supercomputer:Supercomputers`.
