...
the datasets used for the run), and are:
...
- **Profiling folder:** contains the plots of the resource usage of the nodes involved in the execution of the application.
  ...
  tasks executed in that node. Also, when more than one node is involved in the execution, in the folder the user can find
...
- **Enhanced Debugging**: Users can quickly diagnose issues using the included error logs, which detail failed tasks and their underlying causes.
...
.. TIP::
    ...
    The `--tasks` option lists information about each task individually.
...
When the provenance is enabled, the profiling tool is automatically activated. This tool collects the information
about the CPU and memory usage during the execution of the application.
...
will try to use ``top`` command, which is available on most Unix-like operating systems.
...
``/opt/COMPSs/Runtime/scripts/system/profiling/profiler_config.json``.
...
The profiling tool generates the ``csv`` files in a new folder called ``stats/`` of the log directory
(see Section :ref:`Sections/03_Execution/01_Local:Logs` for more details on where to locate these logs).
Here, the user can find a ``csv`` file correspondent to every node involved in the execution of the application,
which contains the CPU and memory usage information of the tasks executed in that node.
...
.. TIP::
    ...
    reduce the profiling overhead, based on the needs of the application and the system. The information
    collected in these files is used to generate the resource usage plots that are included in the provenance
    crate. In the ``stats/`` folder, the user can also find a file called ``stats.json``, which contains the
    main statistics of the resource usage of the environment and tasks in json format.
...
nodes are included. The profiling logs and plots can be useful to detect potential bottlenecks in the application,
inefficient resource usage, and to understand the resource usage of the application during its execution.
