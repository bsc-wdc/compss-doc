...
- ``CreateAction`` details the specific execution of the workflow, compliant with the Workflow Run Crate Profile.
  ...
  - The ``description`` shows the launched command of the execution.
...
- ``resourceUsage`` includes the profiling information of the workflow execution, compliant with the Workflow Run Crate profile. It includes:

  - ``cpuAvg`` represents the average CPU usage of the workflow execution.

  - ``cpuMax`` represents the maximum CPU usage of the workflow execution.

  - ``memAvg`` represents the average memory usage of the workflow execution.

  - ``memMax`` represents the maximum memory usage of the workflow execution.

  - ``memMin`` represents the minimum memory usage of the workflow execution.

  - ``executions`` represents the number of tasks executed in the workflow.

  - ``avgTime`` represents the average execution time of the tasks in the workflow.

  - ``maxTime`` represents the maximum execution time of the tasks in the workflow.

  - ``minTime`` represents the minimum execution time of the tasks in the workflow.

  - ``executionTime`` represents the total execution time of the workflow.
...
``ComputationalWorkflow``, ``version``, ``CreateAction``, ``resourceUsage``), if we first observe the YAML configuration file:
