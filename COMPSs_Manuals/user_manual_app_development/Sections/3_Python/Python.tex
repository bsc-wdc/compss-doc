...
{\it targetDirection} argument of the {\it $@$task} decorator to {\it IN}.

\begin{lstlisting}[language=python]
class MyClass(object):
    ...
    @task(%*{\bf targetDirection }*) = IN)
...
In case a task fails, its failure behaviour can be defined using the {\it on\_failure} argument. It has 
four possible values: 'RETRY', 'CANCEL\_SUCCESSORS', 'FAIL' and 'IGNORE'. 'RETRY' is the default behaviour, 
making the task be executed again, both on the same worker and changing worker if it continues failing. 
'CANCEL\_SUCCESSORS' cancels the following tasks to be executed, 'FAIL' stops the runtime once a task fails and 
'IGNORE' ignores the failure and continues with runtime normal execution.
\begin{lstlisting}[language=python]
...
\bgroup
  ...
  \begin{longtable}{| p{0.28\textwidth} | p{0.72\textwidth} |}
    ...
    \hline
    targetDirection &  INOUT (default), IN or CONCURRENT \\
    \hline
