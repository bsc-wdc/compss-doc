 ...
 \item \textbf{Method-level Metadata:} for each selected method, the following metadata has to be defined:
       \begin{itemize}
		...
		\begin{itemize}
			...
			\item \textbf{onFailure} Expected behaviour if the task fails. \textit{OnFailure.RETRY} makes the task be executed again, 
			\textit{OnFailure.CANCEL\_SUCCESSORS} cancels the following tasks, \textit{OnFailure.FAIL} stops the runtime once a task fails or 
