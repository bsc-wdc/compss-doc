...
For instance, there is a set of decorators that can be placed over the $@$task decorator in order to 
define the task methos as a binary invocation (with the {\it @binary} decorator), as a OmpSs invocation 
(with the {\it @ompss} decorator), or as a MPI invocation (with the {\it @mpi} decorator).
These decorators must be placed over the $@$task decorator, and under the $@$constraint decorator if 
defined.
The following snippets show their usage:

\begin{lstlisting}[language=python]
@binary(binary="mybinary", workingDir="/tmp")
@task()
def binary_func():
     pass
     
@constraint(computingUnits="2")
@binary(binary="otherbinary", workingDir="/tmp")
@task()
def binary_func2():
     pass
\end{lstlisting}
...
\begin{lstlisting}[language=python]
@ompss(binary="ompssApp", workingDir="/tmp")
@task()
def ompss_func():
     pass
\end{lstlisting}
...
\begin{lstlisting}[language=python]
@mpi(binary="mpiApp", workingDir="/tmp", runner="mpirun", computingNodes=2)
@task()
def mpi_func():
     pass
\end{lstlisting}
...
\begin{itemize}
 \item {\it compss\_open(file\_name, mode = 'r')}: similar to the Python {\it open()} call. It synchronizes
       ...
       Python {\it open()}).
 \item {\it compss\_delete(file\_name)}: notifies the runtime to delete a file. 
 ...
 \item {\it barrier()}: performs a explicit synchronization, but does not return any object. The use of {\it barrier()}
       forces to wait for all tasks that have been submitted before the {\it barrier()} is called.
       When all tasks submitted before the {\it barrier()} have finished, the execution continues.
\end{itemize}
...
\bgroup
  ...
  \begin{longtable}{| p{0.45\textwidth} | p{0.5\textwidth} |}
    ...
    \hline
    compss\_delete(file\_name) & Notifies the runtime to remove a file. \\
    \hline
    compss\_wait\_on(obj, to\_write = True) & Synchronizes for the last version of an object (or a list of objects) and returns it. \\
    \hline
    barrier() & Wait for all tasks submitted before the barrier. \\
    \hline
